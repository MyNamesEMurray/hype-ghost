...
**Beta builds** come from `beta-release.yml` (Actions → Beta Release → pick a branch): it tags `vX.Y.Z-beta.N` off the newest *stable* tag, builds the installer, and publishes a GitHub prerelease — testable without merging, and invisible to stable installs because the `-beta.N` suffix makes electron-builder write `beta.yml` rather than the `latest.yml` they poll. Anywhere the next stable version is computed, beta tags **must** be filtered out (`^v[0-9]+\.[0-9]+\.[0-9]+$`): they sort above the release they precede, and splitting `3.8.0-beta.1` on `.` puts `0-beta.1` where a number belongs, which is a fatal arithmetic error rather than a wrong answer.

## Architecture

The app is a local Express + WebSocket server with two hosts:

- **`electron/main.js`** — desktop shell: tray-first (closing the window destroys the renderer but keeps the server chatting), single-instance lock, auto-update from GitHub Releases (restart now / later / skip-this-version — skips persist per version in `update-skip.json` via `src/updateskip.js` and go quiet until a strictly newer release). When packaged, data lives in `%APPDATA%/Hype Ghost/` — unless a `storage.json` pointer there redirects it to a user-chosen folder (`src/storage.js`, surfaced at Settings → About; moving copies the files and relaunches). Packaged builds also tee the console to a rotating `hype-ghost.log` in the data folder (`src/logfile.js`). In dev everything stays in the project root. Saving config from the UI **relaunches the whole app** to apply it (`onConfigSaved`) — unless every changed key is in the server's `HOT_PATHS` set (overlay/theme/cadence/moments/memory/stream/talking points), in which case the server hot-applies it live. Session state (`session.json`: chat history, moments, cost, start time) persists across that relaunch with the same 6-hour freshness rule as session notes.
