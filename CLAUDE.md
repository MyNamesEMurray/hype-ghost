...
- **`electron/main.js`** — desktop shell: tray-first (closing the window destroys the renderer but keeps the server chatting), single-instance lock, auto-update from GitHub Releases. When packaged, data lives in `%APPDATA%/Hype Ghost/` — unless a `storage.json` pointer there redirects it to a user-chosen folder (`src/storage.js`, surfaced at Settings → About; moving copies the files and relaunches). Packaged builds also tee the console to a rotating `hype-ghost.log` in the data folder (`src/logfile.js`). In dev everything stays in the project root. Saving config from the UI **relaunches the whole app** to apply it (`onConfigSaved`) — unless every changed key is in the server's `HOT_PATHS` set (overlay/theme/cadence/moments/memory/stream/talking points), in which case the server hot-applies it live. Session state (`session.json`: chat history, moments, cost, start time) persists across that relaunch with the same 6-hour freshness rule as session notes.
